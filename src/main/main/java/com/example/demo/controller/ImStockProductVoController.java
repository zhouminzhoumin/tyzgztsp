...
*/
@RestController
@RequestMapping("/imstockproductvo/api")
@Api(value = "xx", description = "xx")
public class ImStockProductVoController {
 
    @Resource
    private ImStockProductVoService services;
    ...
    public List<ImStockProductVo> list(HttpServletRequest request) {

    	Condition condition =new Condition(ImStockProductVo.class);
    	Condition.Criteria criteria = condition.createCriteria();
    	criteria.andCondition(" ID ='' ");
    	List<ImStockProductVo> list =services.selectByCondition(condition);
        return  list;
    }
    @ApiIgnore
    ...
    public Object get_one(String id) {
        ImStockProductVo model = services.selectByPrimaryKey(id);
        return model;
        }
        
 	@ApiIgnore
	@ApiOperation(value="新增实体信息",notes="新增实体信息")
	@ApiImplicitParam(name="model",value="实体值", required = true, dataType = "ImStockProductVo")
    @RequestMapping(value = "/add_one", method=RequestMethod.POST)
    @ResponseBody
    public Object add_one(@RequestBody ImStockProductVo model) {
	...
	@ApiImplicitParam(name="model",value="实体集合List<ImStockProductVo>", required = true, dataType = "List<ImStockProductVo>")
    @RequestMapping(value = "/add_multiple", method=RequestMethod.POST)
    @ResponseBody
    public Object add_multiple(@RequestBody List<ImStockProductVo> model) {
    ...
    @ResponseBody
    public Object edit_one(@RequestBody ImStockProductVo model) {
