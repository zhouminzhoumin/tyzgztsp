...
*/
@RestController
@RequestMapping("/ordercrmcodevo/api")
@Api(value = "xx", description = "xx")
public class OrderCrmCodeVoController {
 
    @Resource
    private OrderCrmCodeVoService services;
    ...
    public List<OrderCrmCodeVo> list(HttpServletRequest request) {

    	Condition condition =new Condition(OrderCrmCodeVo.class);
    	Condition.Criteria criteria = condition.createCriteria();
    	criteria.andCondition(" ID ='' ");
    	List<OrderCrmCodeVo> list =services.selectByCondition(condition);
        return  list;
    }
    @ApiIgnore
    ...
    public Object get_one(String id) {
        OrderCrmCodeVo model = services.selectByPrimaryKey(id);
        return model;
        }
        
 	@ApiIgnore
	@ApiOperation(value="新增实体信息",notes="新增实体信息")
	@ApiImplicitParam(name="model",value="实体值", required = true, dataType = "OrderCrmCodeVo")
    @RequestMapping(value = "/add_one", method=RequestMethod.POST)
    @ResponseBody
    public Object add_one(@RequestBody OrderCrmCodeVo model) {
	...
	@ApiImplicitParam(name="model",value="实体集合List<OrderCrmCodeVo>", required = true, dataType = "List<OrderCrmCodeVo>")
    @RequestMapping(value = "/add_multiple", method=RequestMethod.POST)
    @ResponseBody
    public Object add_multiple(@RequestBody List<OrderCrmCodeVo> model) {
    ...
    @ResponseBody
    public Object edit_one(@RequestBody OrderCrmCodeVo model) {
