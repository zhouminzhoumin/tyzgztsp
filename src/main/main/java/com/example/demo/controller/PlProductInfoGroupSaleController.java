 ...
 */
@RestController
@RequestMapping("/plproductgroupsale/api")
@Api(value = "组合上架查询服务")
public class PlProductInfoGroupSaleController {
	...
	@ApiOperation(value="组合上架查询服务",notes="提供商品组合上架信息查询服务，包含同类型商品主信息、商品基本信息、商品上架数量等。目前只支撑同商品型号、同供货商商品的查询结果。")
	@ApiImplicitParam(name="productCode",value="入参为商品编码（唯一7位商品编码标识）", required = true, dataType = "String",paramType = "query")
	@RequestMapping(value = "/productGroupSale_queryService", method=RequestMethod.POST)
	@ResponseBody
	public void productGroupSaleQueryService(@RequestParam String productCode,HttpServletResponse response) throws IOException {
		...
		if(null != productCode && !"".equals(productCode)){
			...
			}else if(productCode.indexOf("LP") >= 0){
