...
*/
@RestController
@RequestMapping("/plproductvo/api")
@Api(value = "xx", description = "xx")
public class PlProductVoController {
 
    @Resource
    private PlProductVoService services;
    ...
    public List<PlProductVo> list(HttpServletRequest request) {

    	Condition condition =new Condition(PlProductVo.class);
    	Condition.Criteria criteria = condition.createCriteria();
    	criteria.andCondition(" ID ='' ");
    	List<PlProductVo> list =services.selectByCondition(condition);
        return  list;
    }
    @ApiIgnore
    ...
    public Object get_one(String id) {
        PlProductVo model = services.selectByPrimaryKey(id);
        return model;
        }
        
 	@ApiIgnore
	@ApiOperation(value="新增实体信息",notes="新增实体信息")
	@ApiImplicitParam(name="model",value="实体值", required = true, dataType = "PlProductVo")
    @RequestMapping(value = "/add_one", method=RequestMethod.POST)
    @ResponseBody
    public Object add_one(@RequestBody PlProductVo model) {
	...
	@ApiImplicitParam(name="model",value="实体集合List<PlProductVo>", required = true, dataType = "List<PlProductVo>")
    @RequestMapping(value = "/add_multiple", method=RequestMethod.POST)
    @ResponseBody
    public Object add_multiple(@RequestBody List<PlProductVo> model) {
    ...
    @ResponseBody
    public Object edit_one(@RequestBody PlProductVo model) {
