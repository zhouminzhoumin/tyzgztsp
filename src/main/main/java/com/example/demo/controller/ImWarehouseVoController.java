...
*/
@RestController
@RequestMapping("/imwarehousevo/api")
@Api(value = "xx", description = "xx")
public class ImWarehouseVoController {
 
    @Resource
    private ImWarehouseVoService services;
    ...
    public List<ImWarehouseVo> list(HttpServletRequest request) {

    	Condition condition =new Condition(ImWarehouseVo.class);
    	Condition.Criteria criteria = condition.createCriteria();
    	criteria.andCondition(" ID ='' ");
    	List<ImWarehouseVo> list =services.selectByCondition(condition);
        return  list;
    }
    @ApiIgnore
    ...
    public Object get_one(String id) {
        ImWarehouseVo model = services.selectByPrimaryKey(id);
        return model;
        }
        
 	@ApiIgnore
	@ApiOperation(value="新增实体信息",notes="新增实体信息")
	@ApiImplicitParam(name="model",value="实体值", required = true, dataType = "ImWarehouseVo")
    @RequestMapping(value = "/add_one", method=RequestMethod.POST)
    @ResponseBody
    public Object add_one(@RequestBody ImWarehouseVo model) {
	...
	@ApiImplicitParam(name="model",value="实体集合List<ImWarehouseVo>", required = true, dataType = "List<ImWarehouseVo>")
    @RequestMapping(value = "/add_multiple", method=RequestMethod.POST)
    @ResponseBody
    public Object add_multiple(@RequestBody List<ImWarehouseVo> model) {
    ...
    @ResponseBody
    public Object edit_one(@RequestBody ImWarehouseVo model) {
