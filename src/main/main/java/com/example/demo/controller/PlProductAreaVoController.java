...
*/
@RestController
@RequestMapping("/plproductareavo/api")
@Api(value = "xx", description = "xx")
public class PlProductAreaVoController {
 
    @Resource
    private PlProductAreaVoService services;
    ...
    public List<PlProductAreaVo> list(HttpServletRequest request) {

    	Condition condition =new Condition(PlProductAreaVo.class);
    	Condition.Criteria criteria = condition.createCriteria();
    	criteria.andCondition(" ID ='' ");
    	List<PlProductAreaVo> list =services.selectByCondition(condition);
        return  list;
    }
    @ApiIgnore
    ...
    public Object get_one(String id) {
        PlProductAreaVo model = services.selectByPrimaryKey(id);
        return model;
        }
        
 	@ApiIgnore
	@ApiOperation(value="新增实体信息",notes="新增实体信息")
	@ApiImplicitParam(name="model",value="实体值", required = true, dataType = "PlProductAreaVo")
    @RequestMapping(value = "/add_one", method=RequestMethod.POST)
    @ResponseBody
    public Object add_one(@RequestBody PlProductAreaVo model) {
	...
	@ApiImplicitParam(name="model",value="实体集合List<PlProductAreaVo>", required = true, dataType = "List<PlProductAreaVo>")
    @RequestMapping(value = "/add_multiple", method=RequestMethod.POST)
    @ResponseBody
    public Object add_multiple(@RequestBody List<PlProductAreaVo> model) {
    ...
    @ResponseBody
    public Object edit_one(@RequestBody PlProductAreaVo model) {
